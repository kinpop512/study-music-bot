...
# Инициализируем бота через переменную окружения, которую ты настроил в Amvera
...
# Наша структурированная база музыкальных концентраторов (VK Видео / RuTube)
# Ссылки ведут на огромные долгоиграющие сборники и стримы без слов
MUSIC_DATABASE = {
    "lofi": {
        "title": "☕ Lo-Fi Фокус (Состояние Потока)",
        "science": "🧠 *Научное обоснование:* Монотонный ритм (70-90 BPM) совпадает с альфа-ритмами мозга. Виниловый треск («розовый шум») блокирует резкие внешние раздражители, помогая войти в состояние глубокой концентрации.",
        "links": [
    ...
    "phonk": {
        ...
        "science": "🔥 *Научное обоснование:* Быстрый, энергичный ритм без слов стимулирует выработку дофамина. Отлично подходит для преодоления прокрастинации и выполнения рутинной, быстрой работы (сборка, верстка, задачи).",
        "links": [
    ...
    "ambient": {
        ...
        "science": "🌊 *Научное обоснование:* Темп ниже 60 BPM снижает уровень кортизола (гормона стресса) и стабилизирует пульс. Полное отсутствие человеческой речи освобождает до 20% ресурсов рабочей памяти мозга.",
        "links": [
...
def send_welcome(message):
    welcome_text = (
        ...
        "Я — *StudyTunesBot*, твой персональный нейро-музыкальный ассистент. "
...
def handle_music_request(call):
    mode_key = call.data.split("_")[1] # Получаем lofi, phonk или ambient
    data = MUSIC_DATABASE.get(mode_key)
    
    if data:
        # Формируем красивый научно-обоснованный ответ
        response_text = f"=== *{data['title']}* ===\n\n{data['science']}\n\n*Рекомендуемые стримы и плейлисты (доступны в РФ):*\n"
        ...
        bot.edit_message_text(
            ...
            text=response_text,
            parse_mode="Markdown",
            disable_web_page_preview=False, # Чтобы красиво подгружалось превью видео прямо в Телеге!
            reply_markup=markup
        )
